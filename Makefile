CURRENT_VERSION := $(shell grep -o "\"version\": \"[0-9]*\.[0-9]*\.[0-9]*\"" package.json | sed 's/"version": "//' | sed 's/"//' || echo "0.0.0")
...
.PHONY: tag-debug
tag-debug:
	@echo "Raw grep result: $$(grep -o 'version: [0-9]*\.[0-9]*\.[0-9]*' package.json || echo 'Not found')"
